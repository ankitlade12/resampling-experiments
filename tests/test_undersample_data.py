...
def test_three_folds(imbalanced_data):
    X, y = imbalanced_data
    xtrainu, ytrainu, xtest, ytest, _, _, _ = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
...
def test_undersampled_folds_are_balanced(imbalanced_data):
    X, y = imbalanced_data
    xtrainu, ytrainu, _, _, _, _, _ = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
...
def test_full_undersampled_set_is_balanced(imbalanced_data):
    X, y = imbalanced_data
    _, _, _, _, _, yu, _ = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
...
def test_test_folds_not_undersampled(imbalanced_data):
    X, y = imbalanced_data
    _, _, _, ytest, _, _, _ = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
    for yt in ytest:
...
def test_xu_smaller_than_original(imbalanced_data):
    X, y = imbalanced_data
    _, _, _, _, Xu, yu, _ = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
    assert len(Xu) < len(X)
    assert len(yu) < len(y)
...
def test_stats_keys(imbalanced_data):
    X, y = imbalanced_data
    _, _, _, _, _, _, stats = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
    assert set(stats.keys()) == {"original_size", "undersampled_size", "removed", "removed_pct"}


def test_stats_values_consistent(imbalanced_data):
    X, y = imbalanced_data
    _, _, _, _, Xu, yu, stats = undersample_data(RandomUnderSampler(random_state=10), X, y, scale=False)
